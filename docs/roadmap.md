...
- **M7d+ 待续**：基础控件扩充 checkbox/slider/progress_bar（M7d）、XML UI 加载器（M8）、y 方向 AA 与 GLES AA（MSAA/顶点覆盖）。**待有 SDK 环境**：iOS(uikit)、HarmonyOS、Android、Web、win32/sdl2 port、Metal backend、FreeBSD/linux_fb 实机复核。
