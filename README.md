...
- **控件系统**：引用计数控件树、linear/flow 布局（px/%/flex；flow 流式换行）、事件分发（grab/焦点/hover 状态机）、主题样式（文本加载 + **CSS 子集**（`class`/`#id`/伪类/后代/级联，docs/css.md））、属性动画、脏矩形局部重绘；内置 button/label/edit（单行输入）/text_area（多行编辑）/checkbox/slider/progress_bar/list_view（虚拟化）/scroll_bar/image/rich_label（行内多段）/scroll_view（通用滚动），复合控件 dialog（模态）/menu（弹出+级联）/tooltip（悬停）
...
- `demo_nodes`：Blender 风格节点编辑器（贝塞尔连线带箭头按类型着色、拖线/磁吸/缩放、框选多选、小地图、连线流动画、节点尺寸自适应、CSS 部件配色）
