...
- **控件系统**：引用计数控件树、linear 布局（px/%/flex）、事件分发（grab/焦点）、主题样式（文本加载）、属性动画、脏矩形局部重绘；内置 button/label/edit（单行输入）/text_area（多行编辑）/checkbox/slider/progress_bar/list_view（虚拟化）/scroll_bar/image，复合控件 dialog（模态）/menu（弹出+级联）/tooltip（悬停）
...
选项（`cmake -LH` 可查）：`MYUI_C_STANDARD=99|11|17|23`、`MYUI_PAL=auto|x11|dummy|linux_fb|wayland`、`MYUI_FONT_STB`、`MYUI_IMAGE_STB`、`MYUI_UI_XML`、`MYUI_BIDI`、`MYUI_BUILD_TESTS`、`MYUI_BUILD_DEMOS`。无 X11 时自动使用 dummy port（headless 可开发可测试）。
