...
static void fb_win_ime_noop(my_pal_window_t* win, int32_t x, int32_t y) {
  (void)win;
  (void)x;
  (void)y; /* linux_fb: no IME */
}

static my_ret_t fb_win_move_noop(my_pal_window_t* win, int32_t x, int32_t y) {
  (void)win;
  (void)x;
  (void)y;
...
static const my_pal_window_vtable_t s_fb_window_vtable = {
    ...
    NULL /* vk_create_surface: fb has no Vulkan (M25b) */};
