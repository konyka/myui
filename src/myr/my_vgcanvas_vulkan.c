...
static void vk_draw_cp(my_vgcanvas_vulkan_t* c, uint32_t cp, float* pen_x,
  ...
  if (c->glyph_cache[slot].tex.img == VK_NULL_HANDLE ||
    ...
    vk_tex_retire(c, &c->glyph_cache[slot].tex);
...
static my_ret_t vk_draw_image(my_vgcanvas_t* vg, const uint8_t* rgba,
  ...
  {
    /* NOTE: translate but NO scale, mirroring the gles2 draw_image path
     * exactly (bug-for-bug; its draw_text does scale) */
    float x0 = dst->x + c->state.tx, y0 = dst->y + c->state.ty;
